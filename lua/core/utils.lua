...
-- Modifies _G._NVIM_CONFIG global variable
M.load_config = function(reload)
    -- only do the stuff below one time, otherwise just return the set config
    if _G._NVIM_CONFIG_CONTENTS ~= nil and not (reload or false) then
        return _G._NVIM_CONFIG_CONTENTS
    end
    ...
    local config_name = vim.g.nvim_user_config or "nvimrc"
    ...
    _G._NVIM_CONFIG_CONTENTS = require(config_name)

    -- user config is not required to run nvim but a optional
    ...
    if vim.fn.filereadable(vim.fn.glob(config_file)) == 1 then
        ...
        if present then
            ...
            if type(config) == "table" then
                ...
                _G._NVIM_CONFIG_CONTENTS =
                    require("core.utils").merge_table(_G._NVIM_CONFIG_CONTENTS, config, to_replace)
            else
